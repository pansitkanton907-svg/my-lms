...
const INIT_MATERIALS = [
  { id:"MAT001", courseId:"CS201",   title:"Week 1: Arrays & Linked Lists", type:"Lecture",    date:"2025-08-05", description:"Introduction to linear data structures" },
  { id:"MAT002", courseId:"CS201",   title:"Week 2: Stacks & Queues",       type:"Lecture",    date:"2025-08-12", description:"Stack and Queue implementations in Python" },
  { id:"MAT003", courseId:"CS301",   title:"Week 1: Classes & Objects",     type:"Lecture",    date:"2025-08-05", description:"OOP fundamentals and class design" },
  { id:"MAT004", courseId:"CS101",   title:"Week 1: What is CS?",           type:"Reading",    date:"2025-08-04", description:"Overview of computer science as a field" },
  { id:"MAT005", courseId:"MATH201", title:"Limits & Continuity",           type:"Assignment", date:"2025-08-08", description:"Practice problems on limits" },
];
...
function StudentCourses({ user, courses }) {
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", height:"100%" }}>
      ...
      <div style={{ flex:1, display:"flex", overflow:"hidden" }}>
        <div style={{ flex:1, padding:"14px 18px", display:"flex", flexDirection:"column", overflow:"hidden", background:"#f8fafc" }}>
          <LMSGrid columns={cols} rowData={myCourses} onRowClick={r=>{setSel(r);setTab("info");}} selectedId={sel?.id} height="100%" />
        </div>
        {sel && (
              ...
              {tab==="info" && (
                <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
                  {[["Teacher",sel.teacherName],["Schedule",sel.schedule],["Units",String(sel.units)+" units"],["Year",sel.yearLevel],["Semester",sel.semester],["Current Grade",sel.grade!=null?`${sel.grade}% (${letterGrade(sel.grade)})`:"Pending"]].map(([l,v])=>v&&(
                    <div key={l}><div style={{ fontSize:9, fontWeight:700, color:"#94a3b8", textTransform:"uppercase", letterSpacing:"0.07em" }}>{l}</div><div style={{ fontSize:12, color:"#334155", marginTop:2 }}>{v}</div></div>
                  ))}
                </div>
              )}
              {tab==="materials" && (
                <div style={{ display:"flex", flexDirection:"column", gap:7 }}>
                  {INIT_MATERIALS.filter(m=>m.courseId===sel.id).length===0
                    ...
                    : INIT_MATERIALS.filter(m=>m.courseId===sel.id).map(m=>(
                      <div key={m.id} style={{ padding:"10px 11px", background:"#f8fafc", borderRadius:6, border:"1px solid #e2e8f0" }}>
                        ...
                        <div style={{ fontSize:10, color:"#94a3b8", marginTop:4 }}>{m.date} · <Badge>{m.type}</Badge></div>
                      </div>
                    ))
                  }
                </div>
