...
import { ProgramCreateRequestDto } from 'src/program/dto/program-create-request.dto';
import { ProgramUpdateRequestDto } from 'src/program/dto/program-update-request.dto';
import { ProgramSearchRequestDto } from 'src/program/dto/program-search-request.dto';
import {
    PageableProgramDto,
    ProgramCountRow,
    ProgramIdRow,
    ProgramOptionRow,
    ProgramRow,
} from 'src/program/dto/program.dto';
...
export class ProgramRepository extends BaseRepository {
    ...
    async create(dto: ProgramCreateRequestDto): Promise<boolean> {
        ...
        const [result] = await this.pool.execute<ResultSetHeader>(
            `INSERT INTO program (code, name, department_id, description)
             VALUES (?, ?, ?, ?)`,
    ...
    async findList(dto: ProgramSearchRequestDto): Promise<PageableProgramDto> {
        ...
        const [cRows] = await this.pool.query<ProgramCountRow[]>(
            `SELECT COUNT(*) AS total FROM program p WHERE ${where}`,
            params
        );
        ...
        return toPaginationDto<ProgramRow>(
            rows, page, size, total, rows.length,
            Boolean(dto.sortParameters?.length)
        );
    }
    ...
    async findActiveOptions(): Promise<ProgramOptionRow[]> {
        const [rows] = await this.pool.query<ProgramOptionRow[]>(
            `SELECT program_id AS programId, name, code
             FROM program
    ...
    async updateById(dto: ProgramUpdateRequestDto): Promise<boolean> {
        ...
        const [result] = await this.pool.query<ResultSetHeader>(
            `UPDATE program
             SET code=?, name=?, department_id=?, description=?, updated_date=NOW()
    ...
    async softDeleteById(programId: number): Promise<boolean> {
        const [result] = await this.pool.query<ResultSetHeader>(
            `UPDATE program SET is_deleted=1, is_active=0
             WHERE program_id=? AND is_deleted=0`,
            [programId]
        );
        return Boolean(result.affectedRows);
